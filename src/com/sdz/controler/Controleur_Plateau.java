				...
				for(int n = 0; n < model.getMatriceLettre()[m].length; n++){
					if(model.getMatriceEtatInt(m, n)==1) {
						...
						if(x1==m) {
							...
							modelMessage.addError("vous pouvez placer des lettres que sur une seul rang�e");
						}
			...
			if(placementbon==true) {
				if(premiertour==true) {
					...
					if(model.getMatriceLettre(7, 7)==null) {
						modelMessage.addError("vous devez posez les premieres lettres � partir du milieu du plateau");
					}
				}
				if(position==1) {
					int k=0;
